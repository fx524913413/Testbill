...
public class PersonEntity_Ab {
    ...
    private int age;
    private Set<TariffStrategyEntity_Ab> tariffStrategies;
    private LocalDateTime nextBillingTime;
    /*    private Set<NormalAccountInfoEntity> normalAccounts;*/
    private Set<AccountInfoEntity_Ab> accounts;
   ...
   /* @OneToMany(cascade = {CascadeType.REMOVE, CascadeType.PERSIST}, targetEntity = NormalAccountInfoEntity.class, mappedBy = "personRelation", fetch = FetchType.LAZY)
    ...
    @OneToMany(cascade = {CascadeType.PERSIST, CascadeType.REMOVE}, targetEntity = AccountInfoEntity_Ab.class/*, mappedBy = "personRelation"*/)
    public Set<AccountInfoEntity_Ab> getAccounts() {
        if (accounts == null)
            accounts = new HashSet<AccountInfoEntity_Ab>();
        return accounts;
    }

    public void setAccounts(Set<AccountInfoEntity_Ab> accounts) {
    ...
    @ManyToMany(targetEntity = TariffStrategyEntity_Ab.class)
    @JoinTable(name = "person_tariffStrategy_table", foreignKey = @ForeignKey(value = ConstraintMode.NO_CONSTRAINT))
    public Set<TariffStrategyEntity_Ab> getTariffStrategies() {
        if (tariffStrategies == null)
            tariffStrategies = new HashSet<>();
        return tariffStrategies;
    }

    public void setTariffStrategies(Set<TariffStrategyEntity_Ab> tariffStrategies) {
